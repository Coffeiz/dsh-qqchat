...
- [ ] 确认当前 DSH Web Conversation renderer 对 `ImageBlock`、`ImageAttachmentRef` 和附件引用的渲染入口；
- [ ] 确认 DSH 原生图片预览、放大、加载失败和空状态组件的公开接口；
- [ ] 确认 DSH 原生文件、音频、视频展示组件是否可以由插件复用；
- [ ] 确认自定义 `qqchat/message` event 是否能直接承载 DSH 原生附件引用；
- [ ] 如果不能承载，记录最小适配边界，禁止在插件中复制 DSH 基础组件。

### B. Host 附件读取和权限

- [ ] 增加 `attachment/read` RPC，仅接受 `attachmentId` 和当前 Session 上下文；
- [ ] 校验 Session 是否映射到当前 QQ account、群或私聊；
- [ ] 校验附件是否属于当前消息、引用消息和会话范围；
- [ ] 校验附件状态、过期时间、媒体类型和读取大小；
- [ ] 图片优先返回 DSH 原生附件引用，而不是 data URL；
- [ ] 只有 DSH renderer 必须读取字节时，才返回短 TTL data URL 或一次性受保护 URL；
- [ ] 不把 QQ 原始签名 URL、AppSecret、Token 或本地文件路径返回给 Client；
- [ ] 为无权限、过期、删除、损坏和超限附件返回稳定的结构化错误。

### C. Agent 消息原生渲染

- [ ] 当前模型支持图片时，将 QQ 图片保存为 DSH `ImageAttachmentRef`；
- [ ] 在同一条 DSH `user/message` 中加入文本块和 `ImageBlock`；
- [ ] 让 DSH Web 原生显示图片，同时让视觉模型直接接收图片；
- [ ] 当前模型不支持图片时，退回 `qqchat_describe_image` 工具；
- [ ] 附件服务不可用、图片损坏或图片超限时，退回文本提示；
- [ ] 保证所有 `ImageBlock` 在 Session event 发布前已经持久提交；
- [ ] 保证 Session event 中不存在 Buffer、Blob、本地路径、SDK 原始对象或循环引用。

### D. 静默 QQ transcript 最小适配

- [ ] 静默消息继续使用 `qqchat/message`，不为了显示媒体改成 `user/message`；
- [ ] event 只携带 JSON 可序列化的附件摘要：ID、类型、文件名、大小、引用标记；
- [ ] 优先寻找 DSH 原生 renderer 消费自定义 event 附件的方式；
- [ ] 如果没有原生入口，再增加无主题的薄适配层；
- [ ] 薄适配层只负责附件引用转换和加载状态，不复制气泡、Modal、按钮或预览组件；
- [ ] 图片加载优先使用 DSH 原生图片组件；
- [ ] 文件、语音、视频优先使用 DSH 原生附件/媒体组件；
- [ ] 加载失败、过期和无权限状态复用 DSH 原生错误/空状态。

### E. 引用消息显示

- [ ] 引用正文、引用发送人和当前正文保持独立数据结构；
- [ ] 引用附件和当前消息附件都使用同一套 DSH 原生附件引用；
- [ ] `quoted` 只表示消息关系，不改变附件权限和渲染主题；
- [ ] 引用图片使用与普通图片相同的原生预览和放大能力；
- [ ] 引用文件、语音和视频使用与普通媒体相同的原生卡片/播放器；
- [ ] 引用附件复用时延长物理附件生命周期，避免引用关系指向已删除文件；
- [ ] 多附件、无文本引用、嵌套引用和引用失效均有明确显示状态。

### F. 媒体类型显示

- [ ] 图片：原生缩略图、原生放大和失败状态；
- [ ] 文件：原生文件名、大小、类型和下载/保存动作；
- [ ] 语音：原生播放条、时长和转写状态；
- [ ] 视频：原生视频入口、时长、尺寸和播放失败状态；
- [ ] 不支持的格式：显示媒体元数据，并引导使用 Agent 工具处理；
- [ ] 不把媒体 URL 直接拼进消息正文；
- [ ] 不让浏览器直接访问 QQ 临时签名 URL。
