...
当前 Host 已提供受权限保护的附件读取 RPC，但仓库中尚未完成自定义 transcript event 到 DSH 原生附件 renderer 的 Client 接入。因此这里的“原生显示”和“薄适配层”仍属于待实现方案，不应视为已经交付。读取失败、附件过期或超出限制时，Host 不返回原始 URL。

#### 文件、音频和视频

这些媒体后续应优先映射到 DSH 已有的附件或文件展示能力，不强行伪装成图片：
...
目前已完成元数据和 Agent 工具侧处理，但尚未完成 DSH Web 的文件、音频、视频 renderer 接入。播放和下载也必须通过有权限、短期有效的附件读取 RPC，不能把 QQ 原始 URL直接暴露给浏览器。
...
- [ ] 引用图片使用与普通图片相同的图片预览路径；原生放大能力待 DSH renderer 公开接口后接入；
- [ ] 引用文件、语音和视频使用与普通媒体相同的媒体卡片；原生播放器待公开接口后接入；
- [x] 引用附件复用时延长物理附件生命周期，避免引用关系指向已删除文件；
- [x] 多附件、无文本引用和引用失效均有稳定的摘要/空状态；嵌套引用按当前 QQ 引用结构扁平化处理。

### F. 媒体类型显示

- [ ] 图片：受保护的缩略图读取、文件名和失败状态；Agent 消息走 DSH 原生图片块；（Host 已完成，Web transcript 仍待接入）
- [ ] 文件：文件名、大小和类型元数据；下载/保存动作待 DSH 文件 renderer 公开后接入；（Host 已完成，Web renderer 未完成）
- [ ] 语音：统一媒体卡片承载类型、文件名和大小；播放/转写待 DSH 原生能力公开后接入；（Host 已完成，Web renderer 未完成）
- [ ] 视频：统一媒体卡片承载类型、文件名和大小；播放/尺寸入口待 DSH 原生能力公开后接入；（Host 已完成，Web renderer 未完成）
...
- [x] 下载 URL、重定向和 DNS 都经过安全检查；
- [ ] 单文件、单消息和视频时长限制生效；（文件和单消息大小已限制，视频时长尚未限制）
- [ ] 附件失败不会留下无限增长的临时文件；
- [ ] 删除消息或 Session 后，附件关系和物理文件按引用计数清理；
- [x] 日志和前端不泄露签名 URL、Token 或文件原文。
...
- [ ] 引用块和当前正文视觉上分离；
- [x] QQ 回复仍然能正确引用触发消息；
- [x] 私聊、群聊、Owner 和普通成员沿用现有权限与 Session 隔离。
