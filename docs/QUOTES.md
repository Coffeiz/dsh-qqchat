...
私聊入站消息仍使用 DSH 原生 `user/message` 事件，以保持原生气泡和会话语义；引用元数据会作为结构化文本前缀保留在该事件中。群聊则使用 QQ 展示事件承载独立引用区域。当前消息附件和引用附件在展示层严格分离，但 Agent 输入和数据库权限范围仍可同时访问两者。

引用区域不能以正文是否为空作为显示条件。纯图片、纯文件等没有文字正文的 QQ 消息仍应显示引用卡片和附件；反思输入也必须保留 `quote.senderId`，避免把被引用人的个人事实归到当前发言人。
