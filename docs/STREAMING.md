...
- [ ] 确认 QQ SDK 的 `stream_messages` 请求和响应字段；
- [ ] 仅为 C2C 私聊创建流式 Writer；
- [ ] 从 DSH assistant text-delta 累计全文；
- [ ] 使用 `replace`，不要直接把 delta 当作独立消息；
- [ ] 为每个流生成新的 `msg_seq`；
- [ ] `index` 从 0 开始严格递增；
- [ ] 首帧响应后再读取和保存 `stream_msg_id`；
- [ ] 所有请求通过串行队列；
- [ ] 采用 300–500ms throttle；
- [ ] 处理 429/50002 重试；
- [ ] 失败时区分“未发送”和“已发送部分”；
- [ ] 增加流式开关和普通发送回退；
- [ ] 增加模拟 HTTP 测试：首帧、后续帧、结束帧、队列竞态、失败降级；
- [ ] 真实 QQ 手机端和桌面端分别验证可见性；
- [ ] 记录 `msg_seq`、`index`、是否有 `stream_msg_id`，但不要记录完整敏感正文。
