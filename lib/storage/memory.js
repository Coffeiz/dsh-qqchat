...
export class MemoryEngine {
    ...
    logger;
    timers = new Map();
    memberTimers = new Map();
    memberBatchTimers = new Map();
    routes = new Map();
    memberRoutes = new Map();
    reflectingGroups = new Map();
    reflectingMembers = new Map();
    reflectingMemberBatches = new Map();
    groupRetryAt = new Map();
    memberRetryAt = new Map();
    memberBatchRetryAt = new Map();
    ...
    dispose() {
        for (const timer of this.timers.values())
            clearTimeout(timer);
        for (const timer of this.memberTimers.values())
            clearTimeout(timer);
        for (const timer of this.memberBatchTimers.values())
            clearTimeout(timer);
        this.timers.clear();
    ...
    setMemberRoute(memberId, provider, model, sessionId) {
        if (!provider || !model)
            return;
    ...
    schedule(groupId) {
        ...
        const timer = setTimeout(() => {
            ...
            if (this.inRetryCooldown(this.groupRetryAt, groupId))
                this.schedule(groupId);
            else
                void this.runScheduledGroupReflection(groupId);
        }, this.reflectionDelay(this.groupRetryAt, groupId));
        timer.unref?.();
        this.timers.set(groupId, timer);
    }
    scheduleMember(memberId) {
        memberId = Number(memberId);
        if (!this.memberRoutes.has(memberId))
            return;
        ...
        const timer = setTimeout(() => {
            ...
            if (this.inRetryCooldown(this.memberRetryAt, memberId))
                this.scheduleMember(memberId);
            else
                void this.runScheduledMemberReflection(memberId);
    ...
    scheduleGroupMembers(groupId) {
        ...
        const timer = setTimeout(() => {
            ...
            if (this.inRetryCooldown(this.memberBatchRetryAt, groupId))
                this.scheduleGroupMembers(groupId);
            else
                void this.runScheduledMemberBatchReflection(groupId);
    ...
    async runScheduledMemberReflection(memberId) {
        try {
    ...
    async runScheduledMemberBatchReflection(groupId) {
        try {
    ...
    async reflectNow(groupId) {
        ...
        return current.finally(() => {
            if (this.reflectingGroups.get(groupId) === current)
                this.reflectingGroups.delete(groupId);
        });
    }
    async reflectNowInternal(groupId) {
        groupId = Number(groupId);
        const route = this.routes.get(groupId);
        if (!route)
            throw new Error('这个群还没有可复用的 DSH 模型路由；先让 Agent 在群里完成一次回复');
        ...
        try {
            ...
            const reflected = await this.generateReflection(route, memorySystemPrompt(), input, `qqchat-memory-${groupId}`);
            ...
            if (lastMessage)
                this.db.markReflected(groupId, Number(lastMessage.id));
    ...
    async reflectMembersNow(groupId) {
        groupId = Number(groupId);
        ...
        return current.finally(() => {
            if (this.reflectingMemberBatches.get(groupId) === current)
                this.reflectingMemberBatches.delete(groupId);
        });
    }
    async reflectMembersNowInternal(groupId) {
        groupId = Number(groupId);
        const route = this.routes.get(groupId);
        if (!route)
            throw new Error('这个群还没有可复用的 DSH 模型路由；先让 Agent 在群里完成一次回复');
        ...
        try {
            const senderIds = new Set(messages
                ...
                .map(message => message.platform_user_id));
            ...
            const lastMessage = messages.at(-1);
            if (lastMessage)
                this.db.markMembersReflected(groupId, Number(lastMessage.id));
            this.db.finishReflectionTask(taskKey, 'completed');
    ...
    async reflectMemberNow(memberId) {
        memberId = Number(memberId);
        ...
        return current.finally(() => {
            if (this.reflectingMembers.get(memberId) === current)
                this.reflectingMembers.delete(memberId);
        });
    }
    async reflectMemberNowInternal(memberId) {
        memberId = Number(memberId);
        const route = this.memberRoutes.get(memberId);
        if (!route)
            throw new Error('这个私聊还没有可复用的 DSH 模型路由；先让 Agent 完成一次回复');
        ...
        const startedAt = Date.now();
        try {
            const reflected = await this.generateReflection(route, privateMemorySystemPrompt(), input, `qqchat-private-memory-${memberId}`);
            ...
            else if (reflected.profile !== undefined)
                this.db.setMemoryDoc('member', memberId, 'profile', stringifyProfile(reflected.profile));
            ...
            else if (reflected.pattern !== undefined)
                this.db.setMemoryDoc('member', memberId, 'pattern', stringifyDoc(reflected.pattern));
            if (reflected.summary !== undefined)
                this.db.setMemoryDoc('member', memberId, 'summary', stringifyDoc(reflected.summary));
            if (reflected.memory !== undefined)
                this.db.setMemoryDoc('member', memberId, 'memory', stringifyMemory(reflected.memory));
            ...
            const lastMessage = messages.at(-1);
            if (lastMessage)
                this.db.setSetting(memberCursorKey(memberId), Number(lastMessage.id));
            this.db.finishReflectionTask(taskKey, 'completed');
    ...
    async compactDaily(scopeType, scopeKey, route, threshold, keepRecent, fallbackSessionId) {
        ...
        try {
            const result = await this.generateReflection(route, system, { text: input }, fallbackSessionId, this.config.memoryCompressionMaxTokens);
            const memory = typeof result?.memory === 'string' ? result.memory.trim() : '';
            if (!memory || !preservesDailyDates(overflow, memory))
                return false;
    ...
    async generateReflection(route, system, input, fallbackSessionId, maxTokens = this.config.memoryMaxTokens) {
    ...
    unreflectedDirectMessages(memberId, limit) {
        const cursor = this.db.getSetting(memberCursorKey(memberId), 0);
    ...
    applyGroupReflection(groupId, reflected) {
        const group = reflected.group || {};
        if (group.profile !== undefined)
            this.db.setMemoryDoc('group', groupId, 'profile', stringifyDoc(group.profile));
        if (group.summary !== undefined)
            this.db.setMemoryDoc('group', groupId, 'summary', stringifyDoc(group.summary));
        if (group.memory !== undefined)
            this.db.setMemoryDoc('group', groupId, 'memory', stringifyMemory(group.memory));
    ...
    applyMemberBatchReflection(groupId, members, reflected, createdAt) {
        ...
        for (const update of updates) {
            ...
            if (update.summary !== undefined)
                this.db.setMemoryDoc('member', memberId, 'summary', stringifyDoc(update.summary));
            if (update.memory !== undefined)
                this.db.setMemoryDoc('member', memberId, 'memory', stringifyMemory(update.memory));
            ...
            if (Array.isArray(update.nicknames)) {
                for (const nickname of update.nicknames) {
                    if (typeof nickname === 'string')
                        this.db.addGroupMemberNickname(groupId, memberId, nickname);
                }
            }
        }
    }
...
export function memberBatchMemorySystemPrompt() {
    return loadReflectionPrompt('member-batch-reflection.md');
}
function memberMemorySystemPrompt() {
    ...
    try {
        ...
        return {
            ...
            attachments: Array.isArray(quote.attachments)
                ? quote.attachments.filter((item) => Boolean(item) && typeof item === 'object')
                : [],
        };
    }
    catch {
...
export function groupCompressionSystemPrompt() {
    return loadReflectionPrompt('group-compression.md');
}
export function memberCompressionSystemPrompt() {
...
export function validateMemberBatchPayload(value, allowedSenderIds) {
    ...
    return members.every(item => {
        ...
        if (typeof senderId !== 'string' || !allowedSenderIds.has(senderId) || seen.has(senderId))
            return false;
        seen.add(senderId);
        return true;
    });
...
function stringifyProfile(value) {
    const entries = normalizeProfileEntries(value);
    return JSON.stringify(entries, null, 2);
}
function mergeProfile(existing, additions, removals) {
...
function mergeTextDoc(existing, additions, removals) {
...
function profileType(value) {
    ...
    if (type === 'name' || type === 'name_observed' || type === 'display_name' || type === 'nickname')
        return 'name';
    if (type === 'address' || type === 'pronoun' || type === 'background' || type === 'dev_env' || type === 'preference')
        return type === 'dev_env' ? 'background' : type;
    return 'note';
}
...
function memberCursorKey(memberId) {
    return `memberReflection:${memberId}`;
}
function boundedMemorySections(parts, budget = MEMORY_INJECT_CHARS) {
    let used = 0;
    const output = [];
    for (const [title, content] of parts) {
        const value = content?.trim() || '';
        if (!value || used >= budget)
            continue;
