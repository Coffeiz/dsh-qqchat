...
function profileTypeForKey(key) {
    if (key === 'name' || key === 'name_observed' || key === 'display_name' || key === 'nickname')
        return 'name';
    if (key === 'address')
        return 'address';
    if (key === 'pronoun')
        return 'pronoun';
    if (key === 'background' || key === 'dev_env')
        return 'background';
    if (key === 'preference')
        return 'preference';
    return 'note';
}
function parseDailyEntries(value) {
    const entries = [];
    let date = '';
...
export class QQChatDatabase {
    ...
    `);
        ...
        const columns = new Set(many(this.db.prepare('PRAGMA table_info(group_members)').all()).map(column => column.name));
        const groupColumns = new Set(many(this.db.prepare('PRAGMA table_info(groups)').all()).map(column => column.name));
        if (groupColumns.has('requires_at'))
            this.db.exec('ALTER TABLE groups DROP COLUMN requires_at');
        if (!columns.has('aliases_json'))
            this.db.exec("ALTER TABLE group_members ADD COLUMN aliases_json TEXT NOT NULL DEFAULT '[]'");
        if (!columns.has('nicknames_json'))
            this.db.exec("ALTER TABLE group_members ADD COLUMN nicknames_json TEXT NOT NULL DEFAULT '[]'");
        if (!columns.has('message_count'))
            this.db.exec('ALTER TABLE group_members ADD COLUMN message_count INTEGER NOT NULL DEFAULT 0');
        const messageColumns = new Set(many(this.db.prepare('PRAGMA table_info(messages)').all()).map(column => column.name));
        const reflectionColumns = new Set(many(this.db.prepare('PRAGMA table_info(reflection_state)').all()).map(column => column.name));
        ...
        if (!messageColumns.has('quote_json'))
            this.db.exec('ALTER TABLE messages ADD COLUMN quote_json TEXT');
    ...
    migrateMemberProfiles() {
        const rows = many(this.db.prepare("SELECT scope_key,content FROM memory_documents WHERE scope_type='member' AND doc_type='profile'").all());
        const update = this.db.prepare("UPDATE memory_documents SET content=?,updated_at=? WHERE scope_type='member' AND scope_key=? AND doc_type='profile'");
    ...
    normalizeDailyDocuments() {
        const rows = many(this.db.prepare("SELECT scope_type,scope_key,content FROM memory_documents WHERE doc_type='daily'").all());
        const update = this.db.prepare('UPDATE memory_documents SET content=?,updated_at=? WHERE scope_type=? AND scope_key=? AND doc_type=\'daily\'');
        for (const row of rows) {
            const entries = parseDailyEntries(row.content);
            const normalized = renderDailyEntries(entries);
            if (normalized !== row.content.trim())
                update.run(normalized, now(), row.scope_type, row.scope_key);
        }
    }
    ...
    getAuthTask(taskId) {
        this.pruneAuthTasks();
        return one(this.db.prepare('SELECT * FROM auth_tasks WHERE task_id=?').get(taskId));
    }
    ...
    upsertAccount(appId, secret, sandbox = false) {
      ...
      sandbox=excluded.sandbox, updated_at=excluded.updated_at`)
        ...
        if (!account)
            throw new Error('QQ account upsert did not return a row');
        return account;
    }
    accountByAppId(appId) {
        return one(this.db.prepare('SELECT * FROM accounts WHERE app_id=?').get(appId));
    }
    accountById(id) {
        return one(this.db.prepare('SELECT * FROM accounts WHERE id=?').get(id));
    }
    firstEnabledAccount() {
        return one(this.db.prepare('SELECT * FROM accounts WHERE enabled=1 ORDER BY id LIMIT 1').get());
    }
    enabledAccounts() {
        return many(this.db.prepare('SELECT * FROM accounts WHERE enabled=1 ORDER BY id').all());
    }
    publicAccounts() {
        return many(this.db.prepare(`SELECT id,app_id,bot_user_id,enabled,sandbox,gateway_status,gateway_last_error,created_at,updated_at
      FROM accounts ORDER BY id`).all());
    }
    setAccountGateway(id, status, error = null, botUserId) {
    ...
    getSetting(key, fallback) {
        const row = one(this.db.prepare('SELECT value FROM plugin_settings WHERE key=?').get(key));
        if (!row)
            return fallback;
        try {
    ...
    upsertGroup(accountId, platformGroupId, defaults = {}) {
      ...
      ON CONFLICT(account_id,platform_group_id) DO UPDATE SET updated_at=excluded.updated_at`)
            .run(accountId, platformGroupId, defaults.name || null, defaults.enabled === false ? 0 : 1, defaults.readEnabled === false ? 0 : 1, t, t);
        const group = this.groupByPlatform(accountId, platformGroupId);
        if (!group)
            throw new Error('QQ group upsert did not return a row');
        return group;
    }
    groupByPlatform(accountId, platformGroupId) {
        return one(this.db.prepare('SELECT * FROM groups WHERE account_id=? AND platform_group_id=?').get(accountId, platformGroupId));
    }
    groupById(id) {
        return one(this.db.prepare('SELECT * FROM groups WHERE id=?').get(id));
    }
    ...
    saveQuoteIndex(input) {
      ...
      ON CONFLICT(account_id,chat_type,chat_id,msg_idx) DO UPDATE SET
        ...
        content=excluded.content,attachments_json=excluded.attachments_json,created_at=excluded.created_at,expires_at=excluded.expires_at`)
            .run(input.accountId, input.chatType, input.chatId, input.msgIdx, input.platformMessageId || null, input.senderId, input.senderName || null, input.content.slice(0, 200), JSON.stringify(input.attachments), input.createdAt, input.expiresAt);
    }
    quoteIndex(accountId, chatType, chatId, msgIdx) {
        const row = one(this.db.prepare(`SELECT * FROM qq_quote_index
      WHERE account_id=? AND chat_type=? AND chat_id=? AND msg_idx=? AND expires_at>?`).get(accountId, chatType, chatId, msgIdx, now()));
        ...
        return {
            id: Number(row.id), accountId: Number(row.account_id), chatType: row.chat_type, chatId: String(row.chat_id),
    ...
    listDirectChats() {
        return many(this.db.prepare(`SELECT mem.*,
    ...
    listKnownMembers() {
        return many(this.db.prepare('SELECT * FROM members ORDER BY last_seen_at DESC').all());
    }
    upsertMember(accountId, platformUserId, displayName = '') {
      ...
      last_seen_at=excluded.last_seen_at`)
        ...
        if (!member)
            throw new Error('QQ member upsert did not return a row');
        return member;
    }
    memberByPlatform(accountId, platformUserId) {
        return one(this.db.prepare('SELECT * FROM members WHERE account_id=? AND platform_user_id=?').get(accountId, platformUserId));
    }
    memberById(id) {
        return one(this.db.prepare('SELECT * FROM members WHERE id=?').get(id));
    }
    touchGroupMember(groupId, memberId, displayName = '') {
        const t = now();
        const existing = one(this.db.prepare('SELECT display_name,aliases_json FROM group_members WHERE group_id=? AND member_id=?').get(groupId, memberId));
      ...
      last_seen_at=excluded.last_seen_at, aliases_json=excluded.aliases_json`).run(groupId, memberId, displayName, t, t, JSON.stringify(aliases));
    }
    listGroupMembers(groupId) {
        return many(this.db.prepare(`SELECT m.id,m.platform_user_id,m.display_name AS global_display_name,
    ...
    addGroupMemberNickname(groupId, memberId, nickname) {
        const value = nickname.trim();
        if (!value)
            return;
        const row = one(this.db.prepare('SELECT nicknames_json FROM group_members WHERE group_id=? AND member_id=?').get(groupId, memberId));
        if (!row)
            return;
    ...
    insertMessage(input) {
        if (input.platformMessageId) {
            const existing = one(this.db.prepare(`SELECT id FROM messages WHERE account_id=? AND platform_message_id=? AND direction=?`)
      ...
      content,quoted_text,mentioned,created_at,raw_json,attachments_json,quote_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`)
            .run(input.accountId, input.platformMessageId || null, input.chatType, input.groupId || null, input.memberId || null, input.direction, input.content || '', input.quotedText || null, input.mentioned ? 1 : 0, input.createdAt || now(), input.raw ? JSON.stringify(input.raw) : null, input.attachments?.length ? JSON.stringify(input.attachments) : null, input.quote ? JSON.stringify(input.quote) : null);
    ...
    linkMessageAttachments(messageId, attachments) {
    ...
    saveAttachment(input) {
      ...
      VALUES(?,?,?,?,?,?,?,?,?,?,?, ?,?)`).run(input.id, input.accountId, input.sourceMessageId || null, input.sourceFileId || null, input.kind, input.filename, input.contentType || null, input.sizeBytes, input.localPath || null, input.imageRef ? JSON.stringify(input.imageRef) : null, 'staged', now(), input.expiresAt || null);
    }
    attachmentById(id) {
        const row = one(this.db.prepare('SELECT id,source_file_id,kind,filename,content_type,size_bytes,local_path,image_ref_json FROM attachments WHERE id=?').get(id));
        if (!row)
            return undefined;
        let imageRef;
        try {
            imageRef = row.image_ref_json ? JSON.parse(row.image_ref_json) : undefined;
        }
        catch { }
    ...
    findReusableAttachment(accountId, sourceMessageId, sourceFileId, kind) {
        const row = one(this.db.prepare(`SELECT id,source_file_id,kind,filename,content_type,size_bytes,local_path,image_ref_json FROM attachments
    ...
    attachmentForSession(sessionId, attachmentId) {
    ...
    expiredAttachmentPaths(before) {
        return many(this.db.prepare("SELECT local_path FROM attachments WHERE expires_at IS NOT NULL AND expires_at<? AND status='expired'").all(before)).map(row => row.local_path).filter((value) => Boolean(value));
    }
    markAttachmentDeleted(path) {
    ...
    groupBySession(sessionId) {
        return one(this.db.prepare('SELECT * FROM groups WHERE dsh_session_id=?').get(sessionId));
    }
    memberBySession(sessionId) {
        return one(this.db.prepare('SELECT * FROM members WHERE dsh_session_id=?').get(sessionId));
    }
    recentGroupMessages(groupId, limit = 40) { return this.listMessages(groupId, limit); }
    unreflectedMessages(groupId, limit = 80) {
        const state = one(this.db.prepare('SELECT last_message_id FROM reflection_state WHERE group_id=?').get(groupId));
        const after = Number(state?.last_message_id || 0);
        return many(this.db.prepare(`SELECT m.*,mem.platform_user_id,mem.display_name
      FROM messages m LEFT JOIN members mem ON mem.id=m.member_id
      WHERE m.group_id=? AND m.id>? ORDER BY m.id ASC LIMIT ?`).all(groupId, after, limit));
    }
    unreflectedCount(groupId) {
        const state = one(this.db.prepare('SELECT last_message_id FROM reflection_state WHERE group_id=?').get(groupId));
        const row = one(this.db.prepare('SELECT COUNT(*) AS n FROM messages WHERE group_id=? AND id>?').get(groupId, Number(state?.last_message_id || 0)));
        return Number(row?.n || 0);
    }
    unreflectedMemberMessages(groupId, limit = 80) {
        const state = one(this.db.prepare('SELECT last_member_reflected_message_id FROM reflection_state WHERE group_id=?').get(groupId));
        const after = Number(state?.last_member_reflected_message_id || 0);
        return many(this.db.prepare(`SELECT m.*,mem.platform_user_id,mem.display_name
      FROM messages m LEFT JOIN members mem ON mem.id=m.member_id
      WHERE m.group_id=? AND m.id>? ORDER BY m.id ASC LIMIT ?`).all(groupId, after, limit));
    }
    unreflectedMemberCount(groupId) {
        const state = one(this.db.prepare('SELECT last_member_reflected_message_id FROM reflection_state WHERE group_id=?').get(groupId));
        const row = one(this.db.prepare('SELECT COUNT(*) AS n FROM messages WHERE group_id=? AND id>?').get(groupId, Number(state?.last_member_reflected_message_id || 0)));
        return Number(row?.n || 0);
    }
    ...
    markMembersReflected(groupId, messageId) {
    ...
    startReflectionTask(input) {
        const existing = one(this.db.prepare('SELECT id,status FROM reflection_tasks WHERE idempotency_key=?').get(input.idempotencyKey));
    ...
    finishReflectionTask(idempotencyKey, status) {
    ...
    memoryDocs(scopeType, scopeKey) {
        const rows = many(this.db.prepare('SELECT doc_type,content,updated_at FROM memory_documents WHERE scope_type=? AND scope_key=?')
    ...
    appendDailyDoc(scopeType, scopeKey, date, note, maxChars = 24_000) {
    ...
    getChatSession(chatType, rowId) {
        ...
        const row = one(this.db.prepare(`SELECT dsh_session_id FROM ${table} WHERE id=?`).get(rowId));
    ...
    dueOutbox(limit = 20) {
        return many(this.db.prepare(`SELECT * FROM outbox WHERE status='pending' AND scheduled_at<=? ORDER BY scheduled_at,id LIMIT ?`).all(now(), limit));
    }
