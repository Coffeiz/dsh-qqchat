...
export function registerQQMediaTools(ctx, db, isAllowed = () => true) {
    const readFileTool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!attachment || !attachment.localPath || !isAllowed(agentId, args.attachment_id))
                return '当前 QQ 消息没有可读取的文件附件。';
            if (attachment.kind !== 'file')
                return '这个附件不是普通文本文件。';
            if (attachment.sizeBytes > MAX_TEXT_BYTES)
                return `文件过大，超过 ${MAX_TEXT_BYTES} 字节读取上限。`;
            try {
    ...
    const mediaInfoTool = defineTool({
        ...
        async execute(args, exec) {
            const attachment = db.attachmentById(args.attachment_id);
            const agentId = String(exec.agent?.id || '');
            if (!attachment || !isAllowed(agentId, args.attachment_id))
                return { ok: false, error: '当前 QQ 消息没有权限访问这个附件。' };
            const info = {
            ...
            if (attachment.localPath) {
                try {
                    info.fileSize = Number((await stat(attachment.localPath)).size);
                }
                catch { }
                if (attachment.kind === 'video' || attachment.kind === 'voice' || attachment.kind === 'audio') {
                    try {
                        ...
                        info.ffprobe = 'ffprobe 不可用或媒体格式无法探测。';
                    }
                }
            }
