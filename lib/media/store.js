...
export class QQMediaStore {
    ...
    logger;
    root;
    constructor(db, ctx, logger = console) {
        this.db = db;
        this.ctx = ctx;
        this.logger = logger;
        this.root = join(dirname(db.path), 'media');
    }
    async ingest(accountId, messageId, attachments) {
        const result = [];
        let messageBytes = 0;
    ...
    async download(input, currentBytes) {
        ...
        for (let redirect = 0; redirect <= 3; redirect++) {
            ...
            if (parsed.protocol !== 'https:')
                throw new Error('QQ media URL must use HTTPS');
            ...
            if (length > MAX_FILE_BYTES || currentBytes + length > MAX_MESSAGE_BYTES)
                throw new Error('QQ media size limit exceeded');
            const data = new Uint8Array(await response.arrayBuffer());
            if (data.byteLength > MAX_FILE_BYTES || currentBytes + data.byteLength > MAX_MESSAGE_BYTES)
                throw new Error('QQ media size limit exceeded');
            return { data };
        }
        return undefined;
    }
    async saveImageRef(input, data) {
        const service = this.ctx?.get('attachments');
        const mediaType = sniffImageType(data);
        if (!service || !mediaType)
            return undefined;
        try {
            return await service.saveImage({ data, mediaType, name: safeFilename(input.filename) });
        }
        catch {
            return undefined;
        }
...
async function assertPublicHost(hostname) {
    ...
    for (const address of addresses) {
        if (isPrivateAddress(address))
            throw new Error('QQ media URL resolves to a private address');
    }
}
function isPrivateAddress(address) {
    if (address === '::1' || address === '0.0.0.0' || address === '::')
        return true;
    if (address.includes(':'))
        return address === '::1' || address.toLowerCase().startsWith('fc') || address.toLowerCase().startsWith('fd') || address.toLowerCase().startsWith('fe8') || address.toLowerCase().startsWith('fe9') || address.toLowerCase().startsWith('fea') || address.toLowerCase().startsWith('feb');
...
function sniffImageType(data) {
    ...
    if (data.length >= 3 && data[0] === 0xff && data[1] === 0xd8 && data[2] === 0xff)
        return 'image/jpeg';
    if (data.length >= 6 && new TextDecoder().decode(data.slice(0, 6)) === 'GIF89a')
        return 'image/gif';
    if (data.length >= 12 && new TextDecoder().decode(data.slice(0, 4)) === 'RIFF' && new TextDecoder().decode(data.slice(8, 12)) === 'WEBP')
        return 'image/webp';
    return undefined;
}
function safeFilename(value) { return basename(value || 'QQ附件').replace(/[^\p{L}\p{N}._-]+/gu, '_').slice(0, 160) || 'QQ附件'; }
...
function inferKind(contentType, filename = '') {
    ...
    if (value.includes('video') || /\.(mp4|mov|avi|mkv|webm)$/u.test(value))
        return 'video';
    if (value.includes('audio') || /\.(mp3|wav|ogg|m4a|amr|silk)$/u.test(value))
        return 'voice';
    if (value.includes('image') || /\.(png|jpe?g|gif|webp|bmp|heic)$/u.test(value))
        return 'image';
    return 'file';
}
