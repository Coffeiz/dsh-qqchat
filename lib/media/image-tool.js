...
export function registerQQImageTool(ctx, db, isAllowed = () => true) {
    const definition = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                if (!value.ok)
                    return [{ type: 'text', text: value.error || '图片不可用。' }];
                if (!value.image_ref || typeof value.image_ref !== 'object')
                    return [{ type: 'text', text: '图片已找到，但当前 DSH 没有可用的图片附件服务。' }];
                return [
                    { type: 'text', text: `已加载图片：${value.filename || value.attachment_id}` },
                    { type: 'image', attachment: value.image_ref },
                ];
            },
        },
