...
export function memorySnapshotHash(text) {
    return createHash('sha256').update(text).digest('hex');
}
export function shouldRefreshMemorySnapshot(previous, currentText, now, ttlMs = MEMORY_SNAPSHOT_TTL_MS) {
    if (!previous || previous.stale)
        return true;
    if (previous.hash !== memorySnapshotHash(currentText))
        return true;
    return now - previous.lastInjectedAt >= ttlMs;
}
export function restoreMemorySnapshotState(events, sourcePlugin) {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
        ...
        if (event.type !== 'user/message' || source?.kind !== 'plugin' || source.plugin !== sourcePlugin)
            continue;
        if (!source.sections?.some(section => section.name === 'qq-chat-context'))
            continue;
