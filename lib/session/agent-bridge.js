...
export function resolveQQSessionPreset(header, events) {
    let current = agentPresetProjectionDefinition.init(header);
    for (const event of events)
        current = agentPresetProjectionDefinition.apply(current, event);
...
export class DshQQBridge {
    ...
    constructor(ctx, db, memory, config, logger = console) {
        ...
        this.disposeToolGate = ctx.on('tools/pre-execute', async (exec, next) => {
            const agent = exec.agent;
            if (!agent)
                return next();
            const actor = this.activeActors.get(String(agent.id));
            if (!actor || actor.chatType !== 'group')
                return next();
            const settings = this.db.runtimeSettings(defaultRuntimeSettings(this.config));
            if (settings.groupMembersCanUseTools)
                return next();
            if (settings.ownerUserId && settings.ownerUserId === actor.senderId)
                return next();
            if (MEDIA_TOOL_NAMES.has(exec.name) && settings.groupMembersCanReadMedia)
                return next();
            return {
                kind: 'deny',
        ...
        const canReadMedia = (agentId, attachmentId) => this.activeMediaReadable.get(agentId) === true && this.activeAttachments.get(agentId)?.has(attachmentId) === true;
    ...
    async attachMappedSessionsToWorkspace() {
        const sessionIds = [
    ...
    async reply(message, group, member, attachments = [], onTextDelta, isOwner = message.chatType === 'c2c') {
        if (message.chatType === 'group' && !group)
            throw new Error('群消息缺少群上下文');
        ...
        return this.serial(key, async () => {
            ...
            try {
                ...
                if (commandText !== undefined) {
                    if (!commands)
                        return '当前 DSH profile 未加载命令系统，请重启并确认使用了最新插件。';
                    ...
                    if (slashCommandName(commandText) === 'qqpreset' && (message.chatType !== 'group' || isOwner)) {
                        ...
                        if (this.pendingResets.delete(String(sessionId)))
                            await this.resetSession(message.chatType, row);
                        return result.text || '';
                    }
            ...
            if (!text)
                throw new Error('DSH Agent 本轮没有产生可发送的文本回复');
            return text;
        });
    }
    onSessionEvent(session, event) {
        ...
        if (event.type === 'request/header') {
            ...
            if (config.provider && config.model)
                this.routes.set(id, { provider: config.provider, model: config.model });
            return;
        }
    ...
    injectMemorySnapshot(agent, sessionId, contextText) {
        const now = Date.now();
    ...
    restoreMemorySnapshot(session) {
        return restoreMemorySnapshotState(session.snapshotEvents(), DSH_RUNTIME_CONTEXT_SOURCE);
    }
    ...
    rememberRoute(chatType, row, agent, sessionId) {
        const provider = agent.options.provider || this.config.provider;
        const model = agent.options.model || this.config.model;
        if (!provider || !model)
            return;
        if (chatType === 'group')
            this.memory.setRoute(Number(row.id), provider, model, sessionId);
        else
            this.memory.setMemberRoute(Number(row.id), provider, model, sessionId);
    }
    appendDisplayIfMissing(session, event) {
        if (session.snapshotEvents().some(item => item.type === 'qqchat/message' && item.data.messageId === event.messageId))
            return;
    ...
    appendOwnerMessageIfMissing(session, event) {
    ...
    ensureTitle(session, chatType, row) {
        try {
            const getService = this.ctx.get;
            const titles = getService?.call(this.ctx, 'sessionTitle');
            if (!titles || titles.get(session))
                return;
            const fallback = chatType === 'group'
                ? `QQ群 · ${row.name || shortId(row.platform_group_id)}`
                : `QQ私聊 · ${row.display_name || shortId(row.platform_user_id)}`;
    ...
    ensureSelection(agent) {
        ...
        if (!route)
            return undefined;
        const ref = { current: route, assembled: undefined };
    ...
    requestReset(sessionId) {
        this.pendingResets.add(sessionId);
    }
    async resetSession(chatType, row) {
    ...
    registerCommands() {
        const register = (name, description, handler, input) => this.ctx.commands.register({ name, description, ...(input ? { input: { hint: input } } : {}), handler });
        const disposers = [
            register('qqhelp', '查看 QQChat 和 DSH 命令', invocation => ({ kind: 'success', text: this.commandHelp(invocation.agent) })),
            register('qqcommands', '查看 QQChat 和 DSH 命令', invocation => ({ kind: 'success', text: this.commandHelp(invocation.agent) })),
            ...
            register('qqpreset', '查看或切换当前 Session 的 Agent Preset', invocation => this.presetCommand(invocation.agent, invocation.rawInput.trim()), 'preset'),
            register('qqstop', '中止当前生成', invocation => {
                if (invocation.agent.status !== 'running')
                    return { kind: 'success', text: '当前没有进行中的生成。' };
            ...
            register('qqstatus', '查看当前会话状态', invocation => this.statusCommand(invocation.agent)),
        ];
        return () => { for (const dispose of disposers.reverse())
            dispose(); };
    }
    async modelCommand(invocation) {
        const agent = invocation.agent;
        const ref = this.selection(agent);
        if (!ref)
            return { kind: 'error', text: '当前 Session 尚未确定模型路由。' };
        ...
        if (!provider || !model)
            return { kind: 'error', text: '用法：/qqmodel provider/model' };
    ...
    async presetCommand(agent, requested) {
        const getService = this.ctx.get;
        const presets = getService?.call(this.ctx, 'agentPresets');
        if (!presets)
            return { kind: 'error', text: '当前 DSH profile 未加载 Agent Preset 系统。' };
        if (!requested) {
    ...
    async persistedPreset(sessionId) {
        const getService = this.ctx.get;
        const persistence = getService?.call(this.ctx, 'sessionPersistence');
        if (!persistence)
            return undefined;
        try {
    ...
    serial(key, task) {
        ...
        return current.finally(() => { if (this.locks.get(key) === current)
            this.locks.delete(key); });
    }
}
function mediaPrompt(attachments) {
...
export function qqMessageContent(text, attachments) {
    const content = [{ type: 'text', text: text + mediaPrompt([...attachments]) }];
...
function transcriptContent(event, db) {
...
function formatDisplayQuote(event) {
...
function displayEventPayload(event) {
...
function extractText(content) {
    return content.filter((block) => isRecord(block) && block.type === 'text' && typeof block.text === 'string').map(block => block.text).join('\n');
}
