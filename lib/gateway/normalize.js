...
export function renderQQMentionNames(text, data, lookup = () => undefined) {
...
export function messageMentionsBot(data, eventType) {
    ...
    if (mentions.length === 0)
        return eventType === 'GROUP_AT_MESSAGE_CREATE';
    for (const item of mentions) {
        if (!item || typeof item !== 'object' || item.bot !== true)
            continue;
        if (Object.prototype.hasOwnProperty.call(item, 'is_you'))
...
function extractQuote(data) {
    ...
    const sender = record?.author && typeof record.author === 'object' ? record.author : undefined;
    ...
    const attachments = extractAttachments(record, true);
    if (!text && !messageId && !senderId && attachments.length === 0)
        return undefined;
    return { messageId, senderId, senderName, text, attachments };
}
function messageSceneIndex(data) {
    const ext = Array.isArray(data.message_scene?.ext) ? data.message_scene.ext : [];
    return sceneValue(ext, 'msg_idx') || String(data.id || '').trim() || undefined;
}
function messageReferenceIndex(data) {
    const ext = Array.isArray(data.message_scene?.ext) ? data.message_scene.ext : [];
    return sceneValue(ext, 'ref_msg_idx') || sceneValue(ext, 'msg_ref_idx') || undefined;
}
function extractAttachments(data, quoted = false) {
    if (!data)
        return [];
    const values = [];
    if (Array.isArray(data.attachments))
        values.push(...data.attachments.filter(isRecord));
    if (Array.isArray(data.msg_elements)) {
        ...
        for (const element of data.msg_elements) {
            if (!quoted && referenceIndex && String(element.msg_idx || '') === referenceIndex)
                continue;
            const type = String(element.type || element.element_type || element.kind || '').toLowerCase();
            if (type && !/(image|photo|picture|file|video|audio|voice|record|media|attachment)/u.test(type))
                continue;
            if (isRecord(element.attachment))
                values.push(element.attachment);
            else
                values.push(element);
        }
    }
    ...
    for (const value of values) {
        ...
        if (!sourceUrl && !platformFileId && !isRecord(value.attachment))
            continue;
...
function numberValue(value, keys) {
    for (const key of keys) {
...
function mediaKind(contentType, filename, rawType) {
    const value = `${contentType || ''} ${filename} ${rawType}`.toLowerCase();
    if (/voice|record|audio|\.(amr|silk|mp3|wav|ogg|m4a)(?:$|\?)/u.test(value))
        return 'voice';
    if (/video|\.(mp4|mov|avi|mkv|webm)(?:$|\?)/u.test(value))
        return 'video';
    if (/image|photo|picture|\.(png|jpe?g|gif|webp|bmp|heic)(?:$|\?)/u.test(value))
        return 'image';
    if (/audio/u.test(value))
        return 'audio';
    return 'file';
}
function sceneValue(ext, key) {
    const prefix = `${key}=`;
