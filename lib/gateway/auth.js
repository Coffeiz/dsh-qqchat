...
export class QQBindService {
    ...
    async start() {
        ...
        if (!response.ok)
            throw new Error(`QQ 创建连接任务失败: HTTP ${response.status}`);
        const payload = await response.json();
        if (payload.retcode !== 0)
            throw new Error(`QQ 创建连接任务失败: ${payload.msg || 'unknown'}`);
        const taskId = payload.data?.task_id;
        if (!taskId)
            throw new Error('QQ 未返回 task_id');
    ...
    async poll(taskId) {
        ...
        if (!response.ok)
            throw new Error(`QQ 轮询授权失败: HTTP ${response.status}`);
        const payload = await response.json();
        if (payload.retcode !== 0)
            return { status: 'fail', reason: payload.msg || 'unknown' };
        ...
        if (!appId || !encrypted)
            return { status: 'fail', reason: 'QQ 返回缺少 AppID 或 Secret' };
        let secret;
        try {
