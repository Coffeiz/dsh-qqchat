...
export function qqCommandText(text, mentioned) {
    const input = text.trim();
    if (/^\/[a-z][a-z0-9_-]*(?:\s|$)/u.test(input))
        return input;
    if (!mentioned)
        return undefined;
...
export function slashCommandName(input) {
    const match = /^\/([a-z][a-z0-9_-]*)(?=\s|$)/u.exec(input);
    return match?.[1];
}
function renderCommandList(commands) {
    if (commands.length === 0)
        return '当前 DSH 没有可用命令。';
...
export async function dispatchQQCommand(commands, agent, input, scope = { chatType: 'c2c', isOwner: true }) {
    if (!input.startsWith('/'))
        return { handled: false };
    const name = slashCommandName(input);
    if (name === undefined)
        return { handled: true, text: '命令格式无效。发送 /help 查看可用命令。' };
