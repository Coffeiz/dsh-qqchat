        ...
        try {
            switch (endpoint) {
                ...
                case 'account/manual-connect': {
                    ...
                    const account = await runtime.connectManual(requireString(payload, 'appId'), requireString(payload, 'appSecret'), record?.sandbox === true);
                ...
                case 'chat/info': {
                    ...
                    if (!member)
                        throw new Error('不是 QQ Chat session');
                    return ok({
                        chatType: 'c2c',
                ...
                case 'group/reflect': {
                    if (!runtime.settings().memoryEnabled)
                        throw new Error('记忆系统已关闭，请先在设置中重新启用');
                    const id = requireNumber(payload, 'groupId');
    ...
    try {
        ...
        return parsed.map(item => {
            if (!item || typeof item !== 'object')
                return null;
            const { localPath: _localPath, ...publicRecord } = item;
            return publicRecord;
...
function requireChatType(payload) {
    const value = asRecord(payload)?.chatType;
    if (value !== 'group' && value !== 'c2c')
        throw new Error('chatType 必须是 group 或 c2c');
    return value;
}
function asRecord(value) {
    return typeof value === 'object' && value !== null ? value : undefined;
}
function asGroupPatch(value) {
    ...
    if (typeof record.readEnabled === 'boolean')
        patch.readEnabled = record.readEnabled;
    return patch;
}
function asSettingsPatch(value) {
    const record = asRecord(value);
    if (!record)
        return {};
    const patch = {};
    if (typeof record.memoryEnabled === 'boolean')
        patch.memoryEnabled = record.memoryEnabled;
    if (typeof record.memoryMemberBatchEnabled === 'boolean')
        patch.memoryMemberBatchEnabled = record.memoryMemberBatchEnabled;
    ...
    if (record.groupReplyFormat === 'smart' || record.groupReplyFormat === 'markdown' || record.groupReplyFormat === 'compat')
        patch.groupReplyFormat = record.groupReplyFormat;
    if (record.directReplyFormat === 'smart' || record.directReplyFormat === 'markdown' || record.directReplyFormat === 'compat')
        patch.directReplyFormat = record.directReplyFormat;
    if (typeof record.directStreamingEnabled === 'boolean')
        patch.directStreamingEnabled = record.directStreamingEnabled;
    if (typeof record.groupMembersCanUseTools === 'boolean')
        patch.groupMembersCanUseTools = record.groupMembersCanUseTools;
    if (typeof record.groupMembersCanReceiveMedia === 'boolean')
        patch.groupMembersCanReceiveMedia = record.groupMembersCanReceiveMedia;
    if (typeof record.groupMembersCanReadMedia === 'boolean')
        patch.groupMembersCanReadMedia = record.groupMembersCanReadMedia;
